...
<li>
    <div class="uk-background-muted uk-padding-small uk-border-rounded">
        ...
        <p class="uk-margin-remove">
		    <?php if ($this->result->route) : ?>
			    <?php echo HTMLHelper::link(
				    ...
				    [
					    'class' => 'f700 font uk-flex uk-h4 uk-text-secondary uk-margin-small-bottom'
				    ]
			    ); ?>
		    <?php else : ?>
	    ...
	    <?php if ($show_description && $description !== '') : ?>
            <p class="uk-margin-remove-top uk-margin-small-bottom uk-margin-bottom font uk-text-secondary uk-text-small">
	    ...
	    <?php if (count($taxonomies) && $this->params->get('show_taxonomy', 1)) : ?>
            <ul class="uk-child-width-auto uk-grid-small uk-grid-divider uk-text-zero" data-uk-grid>
			    <?php foreach ($taxonomies as $type => $taxonomy) : ?>
                    <?php if ($type != 'Author' && $type != 'Language') { ?>
					    ...
					    <?php if ($branch->state == 1 && in_array($branch->access, $user->getAuthorisedViewLevels())) : ?>
						    ...
						    <?php if (count($taxonomy_text)) : ?>
                                <li class="<?php echo $type; ?>">
                                    <span class="uk-text-small uk-text-muted font"><?php echo Text::_(LanguageHelper::branchSingular($type)); ?>&ensp;:&ensp;</span>
                                    <span class="uk-text-small uk-text-secondary font">
                                        <?php echo Text::_($taxonomy_text[0]); ?>
